...
const resetHoverSelected = () => {
    cells.forEach(e => e.classList.remove('hover', 'selected'));
    selected_cell = -1;
}
...
const init = () => {
    ...
    document.querySelector(".main").addEventListener("click", (e) => {
        const grid = document.querySelector(".main-sudoku-grid").getBoundingClientRect();
        const numbers = document.querySelector(".numbers").getBoundingClientRect();
        if (e.pageX > grid.left && e.pageX <= grid.left + grid.width && e.pageY > grid.top && e.pageY <= grid.top + grid.height) {
        } else if (e.pageX > numbers.left && e.pageX <= numbers.left + numbers.width && e.pageY > numbers.top && e.pageY <= numbers.top + numbers.height) {
        } else
            resetHoverSelected();
    });
